...
- Serva Sync requires to be exempted from doze which is asked during the welcome wizard. The app will cease to function if the permission is revoked.
...
Here is how you can reduce battery usage even further by optimizing Serva Sync's settings:

- If you've configured your Serva Sync instances to connect using DynDNS or static IP addresses, turn off all discovery options: Go to "App/Settings/Serva Sync Options" and disable "Local discovery", "Global discovery", "NAT Traversal" and "Relay".
- If you've configured your Serva Sync instances to connect devices which are located on the same local network, turn off global discovery and relay options: Go to "App/Settings/Serva Sync Options" and disable "Global discovery", "NAT Traversal" and "Relay".
...
- Optimize for better battery life
  - Pros:
    - You'll totally love Serva Sync's efficiency.
...
\*1) If you'd like to backup apps or app data, you can use third-party apps like e.g. "App Manager". Serva Sync is not designed to replace an os specific backup utility.
