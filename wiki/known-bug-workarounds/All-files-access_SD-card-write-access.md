...
Android 11 is the first version since 4.4 that supports writing to external SD card again using Serva Sync. That's why the app requests the "access all files and manage external storage" permission on first usage.

### Why SD card use together with Serva Sync requires special attention
...
In Serva Sync, when you are prompted for the directory, you are automatically taken to a writable directory by the built-in directory chooser to help finding out these writable directories with ease. Just create your folder you wish to be used by Serva Sync below the suggested directory that opens up on the UI.

To sum up:

- You CAN write to your internal storage and external SD card using Serva Sync.
- Adoptable Storage counts as "internal" storage.
- You can NOT decide freely where Serva Sync has write access due to the above mentioned Android restriction if the SD card was formatted as PORTABLE.

To ease your technical understanding of the issue and prevent the opening of tickets caused by misunderstandings:

- Serva Sync consists of two components:
