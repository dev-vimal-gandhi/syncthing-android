...
import com.servalabs.sync.util.JobUtils;
...
public class RunConditionMonitor {
    ...
    /**
     * Only relevant if the user has enabled turning Serva Sync on by
    ...
    private class SyncTriggerReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (extraBeginActiveTimeWindow) {
                ...
                mTimeConditionMatch = false;
                /**
                 * If Serva Sync is running and the last run was more than triggeredSyncSleepIntervalS ago,
                 * this stop job might actually start Serva Sync (resp. leave it running) because
                 ...
                 * If Serva Sync really is stopped (which it should) then the wrong time gets
    ...
    /**
     * Determines if Serva Sync should currently run.
