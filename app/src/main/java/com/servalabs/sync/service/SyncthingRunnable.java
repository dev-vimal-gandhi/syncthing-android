...
import static com.servalabs.sync.service.SyncthingService.EXTRA_STOP_AFTER_CRASHED_NATIVE;
...
public class SyncthingRunnable implements Runnable {
    ...
    public enum Command {
        ...
        main,               // Run the main Serva Sync application.
        resetdatabase,      // Reset Serva Sync's database
        resetdeltas,        // Reset Serva Sync's delta indexes
    }

    /**
     * Constructs instance.
     *
     * @param command Which type of Serva Sync command to execute.
     */
        ...
        try {
            ...
            if (returnStdOut) {
                ...
                try {
                    ...
                    Log.w(TAG, "Failed to read Serva Sync's command line output", e);
                } finally {
                    if (br != null)
                        br.close();
                }
            ...
            LogV("Serva Sync exited with code " + exitCode);
            mSyncthing.set(null);
            if (lInfo != null) {
                lInfo.join();
            }
            ...
            switch (exitCode) {
                case 0:
                case 137:
                    Log.i(TAG, "Serva Sync was shut down normally via API or SIGKILL. Exit code = " + exitCode);
                    break;
                case 1:
                    Log.w(TAG, "exit reason = exitError. Another Serva Sync instance may be already running.");
                ...
                case 2:
                    ...
                    Log.w(TAG, "exit reason = exitNoUpgradeAvailable. Another Serva Sync instance may be already running.");
                    mNotificationHandler.showCrashedNotification(R.string.notification_crash_title, Integer.toString(exitCode));
                    sendStopToService = true;
                    break;
                case 3:
                ...
                case 64:
                    ...
                    break;
                default:
                    Log.w(TAG, "Serva Sync exited unexpectedly. Exit code = " + exitCode);
                    mNotificationHandler.showCrashedNotification(R.string.notification_crash_title, Integer.toString(exitCode));
                    sendStopToService = true;
            }
    ...
    private Process setupAndLaunch(HashMap<String, String> env) throws IOException, ExecutableNotFoundException {
        ...
        if (mCommand.length > 0) {
            ...
            if (!libSyncthing.exists()) {
                Log.e(TAG, "CRITICAL - Serva Sync core binary is missing in APK package location " + mCommand[0]);
